...
class AuthServiceImplTest {
    ...
    class reissueTests {
        ...
        void reissue_noUser() {
            ...
            given(request.getCookies()).willReturn(new Cookie[] { refreshCookie });
            given(jwtTokenProvider.getUserId(token)).willReturn(userId);
            given(authRepository.findById(userId)).willReturn(Optional.empty());
        ...
        void reissue_noToken() {
            ...
            given(request.getCookies()).willReturn(new Cookie[] { refreshCookie });
            given(jwtTokenProvider.getUserId(token)).willReturn(id);
            given(authRepository.findById(id)).willReturn(Optional.of(user));

            given(redisService.get("RFT:" + id)).willReturn(null);
        ...
        void reissue_success() {
            ...
            given(jwtTokenProvider.getUserId(token)).willReturn(id);
            given(authRepository.findById(id)).willReturn(Optional.of(user));
            given(redisService.get("RFT:" + id)).willReturn(savedToken);
            ...
            assertEquals(newAccessToken, tokens.getAccessToken());
            assertEquals(newRefreshToken, tokens.getRefreshToken());
            verify(redisService).set("RFT:" + id, newRefreshToken, 7, TimeUnit.DAYS);
        }
    }
